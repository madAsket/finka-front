...
<template>
    <div class="border-b bg-slate-50" v-if="balanceStore.totalBalance !== null">
        <div class="flex text-slate-500 divide-x divide-dashed divide-slate-300 justify-start ml-3   py-1">
            ...
            <div class="pl-3 text-slate-700 grid justify-end items-center ml-auto mr-2 divide-none">
                <p class="text-2xs text-slate-700">Current project:</p>
                <p class="font-extrabold text-2xs">{{projectStore.currentProject.Project.name}}</p>
            </div>
        </div>
    </div>
</template>