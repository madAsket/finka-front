            ...
            <div class="flex items-center justify-start">
                ...
                <div class="flex flex-col sm:w-40 w-32 items-start justify-start">
                    <span class="text-xs font-bold">
                        ...
                        <span class="max-sm:inline-block text-xs font-semibold text-slate-500">
                            <i class="pi pi-database text-xxs text-green-900" ></i>
                            {{ model.Storage.name }}</span>
                    </span>
                    <div class="text-xs max-sm:text-2xs text-slate-500 overflow-hidden pt-1">
                        <p>
                            <Chip :pt="{label:{class:'overflow-hidden max-h-4'}}" 
                            :label="model.ExpenseCategory.name"
                            class="bg-indigo-50 h-5 overflow-hidden"></Chip>
                        </p>
                    </div>
                </div>
            </div>
            <div class="font-light text-2xs text-gray-800 pt-0.5">{{ model.description }}</div>
        </div>
        <div class="flex items-end ml-auto mr-2 flex-col w-28" v-if="projectStore.currentProject.Project.currency !== model.Storage.currency">
            <p class="font-semibold text-sm text-pink-800">-{{ $formatCurrency(model.projectCurrencyAmount, baseCurrency) }}</p>
            <p  class="font-semibold text-xs text-gray-800" >
                {{ $formatCurrency(model.amount, model.Storage.currency) }}
            </p>
        </div>
        <div class="flex items-end ml-auto mr-2 flex-col w-28" v-else>
            <p class="font-semibold text-sm text-pink-800">-{{ $formatCurrency(model.amount, baseCurrency) }}</p>
        </div>
