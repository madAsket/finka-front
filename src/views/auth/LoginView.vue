    ...
    <div class="px-6 py-20 md:px-12 lg:px-20">
        <div class="p-6 shadow rounded-border w-full lg:w-6/12 mx-auto">
            <div class="text-center mb-4">
                ...
                <GoogleAuth class="mb-3" />
                <span class="font-medium leading-normal">Don't have an account?</span>
                <RouterLink to="/signup" class="font-medium no-underline ml-2 text-primary cursor-pointer">Create today!</RouterLink>
            </div>

            <div>
                <form @submit="onLogin">
                    <div class="field mb-6">
                        <InputText id="email" autocomplete="username" v-model="email" type="text" placeholder="Email address" 
                        class="w-full"
                        :class="{ 'p-invalid': errors.email }" />
                        <Message v-if="errors.email"  size="small" severity="error" variant="simple">{{ errors.email }}</Message>
                    </div>
                    <div class="field mb-6">
                        <InputText id="password" autocomplete="current-password" v-model="password" type="password" placeholder="Password" 
                        class="w-full"
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <div class="flex items-center">
                            <Checkbox inputId="rememberme1" v-model="rememberMe" :binary="true" class="mr-2" />
                            <label for="rememberme1">Remember me</label>
                        </div>
                        <RouterLink to="/resetpass" class="font-medium no-underline ml-2 text-primary text-right cursor-pointer">Forgot password?</RouterLink>
                    </div>
